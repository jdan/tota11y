...
class ColorBlindContrastPlugin extends Plugin {
    ...
    getDescription() {
        return `
            Labels elements with insufficient contrast for color blind users
        `;
    }
    ...
    computePerceivedRGB(color, blindness) {
    ...
    runOneType(
        el, combinations, fgColor, bgColor, style, blindness, blindnessName) {
        ...
        fgColor = this.computePerceivedRGB(fgColor, blindness);
        bgColor = this.computePerceivedRGB(bgColor, blindness);
    ...
    run() {
        ...
        $("*").each((i, el) => {
            ...
            for (let blindness of blindnesses) {
                this.runOneType(
                    el, combinations, fgColor, bgColor, style,
                    blindness[0], blindness[1]);
            }
        });
